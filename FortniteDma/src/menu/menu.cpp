...
void menu::PlayerListWindow() {
	...
	if (ImGui::Begin("Player List")) {
		...
		ImGui::BeginChild("ScrollableList", ImVec2(450, 600), true);
	...
	if (selectedPlayer && PlayerList.find(selectedPlayer) != PlayerList.end()) {
		...
		if (ImGui::Begin(std::format("Player Details for -> 0x{:X}", selectedPlayer).c_str(), NULL)) {
			ImGui::Separator();
			...
			ImGui::Text(std::format("Last Submit -> {:.2f}", PlayerList[selectedPlayer].last_submit).c_str());
